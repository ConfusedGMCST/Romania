...
public class officePan : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        if (officeTransform.position.x < leftMax.x && Input.mousePosition.x < Screen.width/7) {
            if (leftFrame < 0.5)
            {
                leftFrame += 0.02f;
            }
        ...
        if (officeTransform.position.x > rightMax.x && Input.mousePosition.x > (Screen.width / 7) * 6)
        {
            if (rightFrame < 0.5)
            {
                rightFrame += 0.02f;
